...
# Compute F on the cpu
cpu_result = F(x, dim=0)

# Send input tensor to device
x_device = x.to(DEVICE)

# Compute F on the device in eager mode and get the result back to the host
# eager_result = F(x_device, dim=0).cpu()

# Compute F on the device in compiled mode and get the result back to the host
compiled_f = torch.compile(lambda a: F(a, dim=0))
compiled_result = compiled_f(x_device).cpu()
