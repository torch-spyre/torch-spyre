...
`core_divisions` with. Two more depend on the menu without reading `menu_index`: a buffer is
flippable only when its menu holds more than one entry, and the recolor anchor is drawn from the
enumerated non-trivial configs — so today a buffer with a single-entry menu cannot move even where
an alternative is generable, and an anchor is always a menu config. Everything outside those five
reads the config, so a division the engine *generates* rather than enumerates already works there.
Note that a menu carries the same choice at several positions — a factor-1 axis is dropped from the
sparse split map, so `{d0: 2, d1: 1}` enumerates as a second copy of `{d0: 2}` — and two such
configs compare equal while their menu positions differ.
