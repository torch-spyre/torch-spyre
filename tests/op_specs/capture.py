...
def capture_kernels(
    ...
    def spy(method: str):
        ...
        def wrapper(self, kernel_name, specs, pool_size=0):
            rec = KernelRecord(
                name=kernel_name,
                specs=list(specs),
                index=len(records),
            ...
            records.append(rec)
            # ktir() takes no pool_size: that emitter refuses an hbm_pool buffer,
            # so the scheduler never emits one for it.
            if method == "ktir":
    ...
    with contextlib.ExitStack() as stack:
        ...
        if no_execute:
            ...
            # The KTIR prerequisite check goes too: it demands dbo-opt on PATH,
            # which a capture that compiles nothing does not need.
