...
class TestModuleCustom(TestCase):
    ...
    def test_eager_vs_compile(self, device, dtype, module_info, training):
        ...
        for module_input in module_inputs:
            ...
            module_cpu = module_info.module_cls(*module_input.constructor_input.args,
                                                 **module_input.constructor_input.kwargs)
            module_cpu.eval()
            
            # Create module on device (eager)
            module_device_eager = module_info.module_cls(*module_input.constructor_input.args,
                                                          **module_input.constructor_input.kwargs).to(device)
            ...
            module_device_compile_base = module_info.module_cls(*module_input.constructor_input.args,
                                                                **module_input.constructor_input.kwargs).to(device)
            ...
            args_device = tuple(arg.to(device) if isinstance(arg, torch.Tensor) else arg 
                               for arg in args_cpu)
            kwargs_device = {k: v.to(device) if isinstance(v, torch.Tensor) else v 
            ...
            with torch.no_grad():
                ...
                output_device_compile = module_device_compile(*args_device, **kwargs_device)
            ...
            if (output_cpu_tensor is not None and isinstance(output_cpu_tensor, torch.Tensor) and
                output_device_eager_tensor is not None and isinstance(output_device_eager_tensor, torch.Tensor) and
                output_device_compile_tensor is not None and isinstance(output_device_compile_tensor, torch.Tensor)):
    ...
    def test_layout(self, device, dtype, module_info, training):
        ...
        for module_input in module_inputs:
            module = module_info.module_cls(*module_input.constructor_input.args,
                                           **module_input.constructor_input.kwargs).to(device)
            module.eval()
            ...
            self.assertIsNotNone(output_contiguous, 
                               f"{module_info.name}: Contiguous layout test failed")
            ...
            if args and isinstance(args[0], torch.Tensor) and args[0].ndim == 4:
                ...
                self.assertTrue(
                    args_channels_last[0].is_contiguous(memory_format=torch.channels_last),
                ...
                self.assertIsNotNone(output_channels_last,
    ...
    def test_stride(self, device, dtype, module_info, training):
        ...
        for module_input in module_inputs:
            ...
            self.assertIsNotNone(output_default,
                               f"{module_info.name}: Default stride test failed")
            ...
            if args and isinstance(args[0], torch.Tensor):
                ...
                if tensor.shape[-1] > 2:  # Need at least 3 elements to slice
                    ...
                    if original_stride != new_stride:
                        ...
                        self.assertIsNotNone(output_strided,
            ...
            if args and isinstance(args[0], torch.Tensor) and args[0].ndim >= 2:
                ...
                if tensor.shape[0] > 1:
                    ...
                    if expanded.stride()[0] == 0:
                        ...
                        self.assertIsNotNone(output_broadcast,
