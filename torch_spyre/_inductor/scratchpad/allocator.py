...
class CoOptimizingAllocator(ScratchpadAllocator):
    ...
    def _build_cd_bound_buffers(
        ...
        *total* device footprint plus its producer edges (``parent_proj``); the
        ...
        the whole legal space -- its op's split space, built with this buffer's
        own cost symbols so a generated division stays priceable.
        """
        ...
        for output_name, info in mem_usage.items():
            ...
            cd_parent_matches = {
                parent: edge.match_pairs(divisions[parent], buf_divisions)
            ...
            if op is not None and output_name in division_map.enumerated:
    ...
    def _solver_chooses_tilings(self) -> bool:
        ...
        Which engine it is *is* the switch, as far as a user is concerned. Only
        ``SaCoOptimizingSolver`` generates divisions rather than indexing the
        enumerated list, and only a generated division can carry a ``TileSpec``
        -- the enumeration has none to offer, so the CP-SAT and DFS engines
        could not choose one if they wanted to. ``select_allocator`` reaches
        this solver from exactly two settings (``co_optimizing_lx_planning``
        plus ``layout_solver = "simulated_annealing"``); a separate flag on top
        ...
        """
        return TILE_CHOICES_ARE_APPLIED and self.layout_planning is SaCoOptimizingSolver
    ...
    def _cd_parent_matches(
        ...
        """
        return {
            parent: edge.match_pairs(divisions[parent], consumer_divs)
