...
def _table_relation(
    ...
    key_pairs = {
        (_split_key(parent_keys[ip]), _split_key(child_keys[ic]))
        for ip, ic in pairs
    ...
    parent_config = {_split_key(key): config for key, config in parent_by_key.items()}
    child_config = {_split_key(key): config for key, config in child_by_key.items()}
    position = {
