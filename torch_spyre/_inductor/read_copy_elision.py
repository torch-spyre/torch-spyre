...
def _prove_matmul_direct_read(
    ...
    if is_matmul:
        # A weight layout may permute device axes. Prove their identity from
        # the reads before comparing the exact owner map; split counts alone
        # do not identify the weight slice.
        try:
            matching_owners = _views_match_through_read_coordinates(
                ...
                device_coordinates(source_layout.device_layout, source_dep, None),
            )
        except Unsupported as exc:
            return None, f"weight layout identity is not provable: {exc}"
    else:
        # Preserve the pointwise direct-read proof introduced by #4558.
        matching_owners = _direct_source_ownership_matches(
