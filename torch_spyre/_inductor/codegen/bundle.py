...
def generate_bundle(
    ...
    # Deduplicated by arg_index: multiple SDSCs may reference the same kernel arg
    # at different HBM offsets (e.g. different slices of the same tensor), so we
    # keep only the first sym_idx for each unique arg_index and map subsequent
    # occurrences to it.
